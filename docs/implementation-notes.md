...
Replaced the foreground-only product assumption with iOS 26 continued processing. Each active incomplete torrent registers and submits a `BGContinuedProcessingTaskRequest` whose identifier is keyed by the info hash when it is added or resumed. Restored torrents register during restoration, then submit once restoration is complete and the SwiftUI scene is fully active. The task observes the existing `StatusBroadcast`, reports byte-accurate verified progress and download rate through the system UI, completes when every piece verifies, and pauses the torrent so peer sockets close and resume state persists when the task expires or the user cancels it. Pausing or deleting a torrent cancels its request. Download payloads, nested directories, and verified-piece sidecars use `completeUntilFirstUserAuthentication` protection so an active task retains storage access after the phone locks. Background seeding remains disabled.
...
Verification: all 80 Swift tests pass; `stupid-app build` produces an iOS 26.0 binary with the expected background modes and permitted identifier in the packaged plist; the app was reinstalled and launched on the iOS 26.3 simulator. Simulator returned the documented `BGTaskSchedulerErrorDomain error 1` because it does not support background processing, so final validation used `stupid-app run --network` on a physical iPhone running iOS 26.6. The first physical run exposed a Swift actor-isolation crash because BackgroundTasks invoked the inherited actor-isolated launch closure on its dispatch queue; nonisolated launch/expiration adapters now synchronously install provisional progress and the expiration handler, then hop into `ContinuedDownloadManager`. A second device run found that a pause operation could cancel a newly resumed task after the UI already displayed Paused; cancellation now completes before `Torrent.pause()` publishes that state. Transfer rates also retained the last pre-pause value because the timer-driven sampler did not reliably wake through the physical background transition; pause/resume now reset its baseline, elapsed time is included in the rate calculation, and throttled block receipts provide a once-per-second fallback status sample. With these fixes, an explicit Pause -> Resume produced a persistent system activity, verified torrent progress continued for more than 30 seconds after the Home gesture, and speed updated in both the activity and detail view. Submitting restored tasks directly inside restoration still caused iOS to report `Task Failed` when minimized because the scene had not completed activation; deferring submission until both restoration completion and `scenePhase == .active` fixed that path. The final startup test confirmed a restored torrent created persistent system activity and continued progressing for more than 30 seconds after Home without Pause/Resume; a separate lock test confirmed progress and speed continued through a 30-second phone lock. Build 7 was uploaded to TestFlight and reached `READY_FOR_BETA_TESTING`.
...
- `MKVHLSStream` prepares from the verified Matroska head and Cues, selects the same retained default video/audio tracks as `MKVRemuxer`, publishes a complete VOD playlist and init segment, and generates only requested cue-to-cue media ranges. Segment generation is serialized so HLS lookahead cannot replace the active torrent priority window, cancellation stops unavailable reads, legal non-Cluster Segment children are skipped structurally, malformed timelines fail loudly, and segment/cache sizes are bounded.
- `HLSLoopbackServer` binds only `127.0.0.1`, uses unique session paths, supports GET/HEAD and single byte ranges, detects disconnected clients while target pieces are pending, and sends media without building another full response copy. `TorrentStreamSession` retains one server/asset, waits for the prioritized Cues index on partial downloads, and makes stop/cancellation terminal so a dismissed view cannot recreate playback.
- `TorrentItem` retains asynchronous per-file MKV capability checks so detail-view recreation cannot lose them. Partial MKV play buttons remain disabled until HLS preparation validates usable Cues; missing/invalid Cues keep the button disabled until the torrent completes. Completed MKVs can use the exact complete-file fMP4 path. The app-owned `SegmentedPlayerView` and production replacement-item selection were removed, so every player the UI can open uses native controls.
- Regression coverage now proves sparse head+Cues preparation, rejection of a Cues-less partial MKV, no reads from an intervening media interval, local `tfdt` reset at each HLS segment boundary, a 30.023-second AVFoundation timeline, two completed seeks on the same item with playback advancing after both, HTTP HEAD/range behavior, and cancellation while Cues are unavailable. The full suite has 77 tests across 16 suites.
- Throttled production harness: `long30.mkv` started HLS playback with 5/11 pieces initially verified, fetched the sparse Cues piece, played while 7/11 were verified, then a seek to 25 seconds prioritized the target tail and resumed at 25.9 seconds after those pieces verified (`SEEK OK`).
- iOS 26.3 simulator with native `AVPlayerViewController`: the MTP item showed the full `1:47:39`; native scrubs decoded visibly different frames at `20:19` and `1:05:34`; the second native clock continued past `1:05:59` without resetting or replacing the global timeline. After resume revalidation left the torrent genuinely partial at 96/1016 pieces (9%), the row capability check enabled its button and opening it still produced native AVKit controls, a full `1:47:39` timeline, and advancing video. `xtool dev run --simulator` built and installed successfully.
