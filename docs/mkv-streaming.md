...
- **Render smoke test (temporary, removed after passing)**: an env-gated auto-open path played generated H.264/AAC MKVs (ffmpeg `testsrc`+`sine`) through `MKVStreamSession`. VLC reported advancing playback with a 60.023 s duration, and a follow-up visual check on the iOS 26.3 simulator showed the test pattern and timeline advancing; screenshots six seconds apart changed 31,029 pixels. Full render + playback is proven, and the earlier static screenshots were not a drawable failure.
