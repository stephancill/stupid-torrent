...
5. MKV/MKA stream through **AVPlayer** via an in-house Matroska → fragmented-MP4 transmuxer (`Streaming`: EBML parse → fMP4 mux, served through the same `AVAssetResourceLoaderDelegate`). Covers H.264/HEVC video and AAC/E-AC-3/AC-3/ALAC audio (E-AC-3/AC-3 decode verified on macOS + iOS 26.3 sim); embedded subtitles and Opus/Vorbis/FLAC/DTS are dropped (see `docs/mkv-avplayer-transmuxer.md`). Other formats (webm/avi/AV1-in-MKV) remain unplayable.
